    ...
    : ZKbMainWidget ( ZHeader::FULL_TYPE, NULL, "ZMainWidget", 0 )
{
  ...
  QString val; ZConfig PREini(QString("%1/z6manager.cfg").arg(ProgDir), false);
  val = PREini.readEntry(QString("Z6Manager"), QString("Language"), "");
  if ( val == "" ) {
	...
	system(QString("echo \"CURRENT LANGUAGE = %1\"").arg(Idioma));
  } else {
...
void Z6Man::LimpiarCR()
{
  ...
  QString card2 = QString("/ezxlocal/download/appwrite/am/UserRegistry");
  QString line2="";
  ...
  system("mv /ezxlocal/download/appwrite/am/UserTemp /ezxlocal/download/appwrite/am/UserRegistry");
...
void Z6Man::CargarListaSkins()
{
  ...
  if ( entrada.open(IO_ReadOnly | IO_Translate) )
  {
	...
	{           
		...
		if ( IDval == "Alkali" ) { listitem->appendSubItem ( 1, IDval2, false, p2 ); }
		else { 
...
void Z6Man::BorrarMPKG()
{
  ...
  if (i)
  {
	...
	else { card ="/ezxlocal/download/appwrite/am/UserRegistry"; }

	system(QString("sed -n -e '/^AppID =/p' /mmc/mmca1/.system/java/CardRegistry /ezxlocal/download/appwrite/am/UserRegistry | awk '{print $3}' | sed 's/{//g' | sed 's/}//g' > %1/TempApps").arg(ProgDir));


	QFile napps(QString("%1/TempApps").arg(ProgDir)); 
	...
	if ( napps.open(IO_ReadOnly | IO_Translate) )
	{
	  ...
	  {           
		line = sapps.readLine();
		QString val, val2; ZConfig ini(card, false);
	...
	napps.close();
	system(QString("rm %1/TempApps").arg(ProgDir));
	qApp->processEvents();
...
void Z6Man::Instalar(QString mpkg)
{
	...
	system(QString("/ezxlocal/LinXtend/usr/bin/busybox2 tar xf \"%1\" -C /mmc/mmca1/.system/tempmpkg/").arg(mpkg));  //For use with new busybox
	...
	system(QString("sed -n -e '/^Name =/p' /mmc/mmca1/.system/java/CardRegistry /ezxlocal/download/appwrite/am/UserRegistry | awk '{print $3}' | sed 's/{//g' | sed 's/}//g' > %1/TempApps").arg(ProgDir));

	QFile napps(QString("%1/TempApps").arg(ProgDir)); 
	QTextStream sapps(&napps);
	QString line; QString installed = "NO";

	if ( napps.open(IO_ReadOnly | IO_Translate) )
	{ while ( !sapps.eof() ) { line = sapps.readLine(); if ( line == val ) { installed = "SI"; } } }
	system(QString("rm %1/TempApps").arg(ProgDir));
	...
	else
	{
		...
		QString IDval = IDini.readEntry(QString("Z6MANAGER"), QString("INSTALL_INSTALLED"), "");
	 	ZMessageDlg* msg =  new ZMessageDlg ( QString::fromUtf8(APPTITLE), IDval, ZMessageDlg::just_ok, 0, NULL, "ZMessageDlg", true, 0 );
		msg->exec();
		system("rm -r /mmc/mmca1/.system/tempmpkg");
		delete msg;
		msg = NULL;
...
void Z6Man::InstalarApp(QString mpkg, QString name)
{
	...
	QString Lugar = "Nada";
    ZConfig IDini(Idioma, false);
	QString IDval = IDini.readEntry(QString("Z6MANAGER"), QString("INSTALL_PLACE_APP"), "");
	...
	if ( Lugar != "Nada" )
	{
	  ...
	  msg0->exec();
		...
		QString menu ="2a58c0d6-05c6-45aa-8938-63059d41a4e9";

		ZConfig UMT("/ezxlocal/download/appwrite/am/UserMenuTree", false);
		QString UMTval = UMT.readEntry(QString("2a58c0d6-05c6-45aa-8938-63059d41a4e9"), QString("Items"), "");
		QString valor2 = QString("%1;%2").arg(AppID).arg(UMTval);
		UMT.writeEntry(QString("2a58c0d6-05c6-45aa-8938-63059d41a4e9"), QString("Items"), valor2);
		...
		if ( Lugar != "Telefono" )
		{ system(QString("chmod 777 /mmc/mmca1/.system/QTDownLoad/*/*").arg(ProgDir)); }
		else
		{ system(QString("chmod 777 /ezxlocal/download/mystuff/.system/QTDownLoad/*/*").arg(ProgDir)); }
	  ...
	  ZMessageDlg* msg =  new ZMessageDlg ( QString(APPTITLE),QString("%1 %2").arg(valor).arg(val),
		ZMessageDlg::yes_no, 0, NULL, "ZMessageDlg", true, 0 );
	  int i = msg->exec(); delete msg; msg = NULL;
	  if (i) { system("busybox killall am"); }
	}
		...
		if ( zopen->result() == 1 )
		{
		  ...
		  if (archivo != "")
		  {
			...
			for ( QStringList::Iterator it = grouplist.begin(); it != grouplist.end(); ++it ) {
				...
				if ( name==currentItem ) {
					QPixmap p1; QString dir, ico;
					dir = confCARD.readEntry ( *it, "Directory", "" );
					ico = confCARD.readEntry ( *it, "BigIcon", "" );
					if ( ! QFileInfo(dir + "/" + "iconbackup.png").isFile()) { 
						if ( attr==5 ) { p1.load(dir + "/" + ico); } else { p1.load(ico); }
						p1.save(dir + "/" + "iconbackup.png", "PNG"); }
					p1.load(archivo); 
