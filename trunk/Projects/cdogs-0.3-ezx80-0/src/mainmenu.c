...
static struct Credit credits[] = {
	...
	//EZX
	{"Cao Teng",
	 "Finally port this game to our EZX! ;)"}
};
...
static void ShowAllKeys(int index, int change)
{
	DisplayKeys(75, 230, 20, "Player One", &gPlayer1Data, index,
		    change);
	TextStringAt(75, 90, "Map");
	if (change == 6)
		DisplayMenuItem(230, 90, SELECTKEY, index == 6);
	else
		DisplayMenuItem(230, 90, SDL_GetKeyName(gOptions.mapKey),
				index == 6);
	DisplayMenuItem(75, 100, "Done", index == 7);
}
