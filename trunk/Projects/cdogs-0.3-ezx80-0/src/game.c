...
static void MissionStatus(void)
{
	...
	for (i = 0; i < gMission.missionData->objectiveCount; i++) {
		...
		if (gMission.missionData->objectives[i].required > 0) {
			...
			scr[x + 235 * 320] = color;
			scr[x + 1 + 235 * 320] = color;
			scr[x + 1 + 234 * 320] = color;
			scr[x + 234 * 320] = color;
			...
			if (left > 0) {
				if ((gMission.missionData->objectives[i].
					...
					strcpy(s, "?");
				TextStringAt(x + 5, 230, s);
				allDone = 0;
			} else
				TextStringAt(x + 5, 230, "Done");
			x += 25;
		}
	}
...
void StatusDisplay(void)
{
	...
	if (gameIsPaused) {
		if (escExits)
			TextStringAt(90, 80, "Press Start again to quit");
		else
