...
int clipleft = 0, cliptop = 0, clipright = 319, clipbottom = 239;
...
void CopyToScreen(void)
{
	...
	{
		memcpy(pScreen, r_screen, 76800);	//320*240
	}
	else {
		int i;
...
void SetPalette(void *pal)
{
	...
	for (i = 0; i < 256; i++) {
		newpal[i].r = palette[i].red * 4;
		newpal[i].g = palette[i].green * 4;
		newpal[i].b = palette[i].blue * 4;
		newpal[i].unused = 0;
	}
