...
////////////////////////////////////////////////////////////////////////
// GPU Plugins functions 
////////////////////////////////////////////////////////////////////////

long GPU_init(void)
{
	memset(ulStatusControl,0,256*sizeof(unsigned long));  // init save state scontrol field
	memset(&PSXDisplay, 0, sizeof(PSXDisplay_t));
	memset(&PreviousPSXDisplay, 0, sizeof(PSXDisplay_t));
...
long GPU_open(unsigned long * disp, char *, void *)
{
	unsigned long d;
...
// eon
		...
		PSXDisplay.DisplayEnd.y = PSXDisplay.DisplayPosition.y+ PSXDisplay.DisplayMode.y + PreviousPSXDisplay.DisplayModeNew.y;
		PreviousPSXDisplay.DisplayEnd.x = PreviousPSXDisplay.DisplayPosition.x+ PSXDisplay.DisplayMode.x;
		PreviousPSXDisplay.DisplayEnd.y = PreviousPSXDisplay.DisplayPosition.y+ PSXDisplay.DisplayMode.y + PreviousPSXDisplay.DisplayModeNew.y;
...
// NEWX
	if(!PSXDisplay.Interlaced && UseFrameSkip) {            // stupid frame skipping
		...
		if(VRAMWrite.x<PSXDisplay.DisplayEnd.x && VRAMWrite.Width >=PSXDisplay.DisplayPosition.x && VRAMWrite.y<PSXDisplay.DisplayEnd.y && VRAMWrite.Height>=PSXDisplay.DisplayPosition.y)
			updateDisplay();
	}
*/
...
void GPU_writeData(unsigned long gdata)
{
...
void GPU_writeDataMem(unsigned long * pMem, int iSize)
{
...
void GPU_readDataMem(unsigned long * pMem, int iSize)
{
	int i;
...
long GPU_dmaChain(unsigned long * baseAddrL,unsigned long addr)
{
...
long GPU_freeze(unsigned long ulGetFreezeData, GPUFreeze_t *pF)
{
	//----------------------------------------------------//
