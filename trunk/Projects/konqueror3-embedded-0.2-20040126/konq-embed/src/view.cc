...
{
    if ( !frame || frame->zoomFactor() == m_zoom )
	return;

    frame->setZoomFactor( m_zoom );

    const QPtrList<KParts::ReadOnlyPart> children = frame->frames();
...
void BrowserView::openURL( const KURL &url, const QString &mimeType, const KParts::URLArgs &_args )
{
	...
	emit createPartView( url, mime->isPart()->getFactory(),
			     mime->isPart()->isReadWrite() );
	return;
    }
...
{
    m_doc->findTextNext( findText(), true, false, false );
}
