...
bool Config::ParseConfigFile(char const * const configfilename){
	...
	char line[BUFSIZ];
	int lineno;
	rgb = fopen(configfilename,"r");
	lineno=0;
	    /*while(fgets(line,sizeof(line), rgb))
		{
		++lineno;
		printf("%d:%s",lineno,trim(line));
		}*/
	...
	//myfile1.write("PUTO",1);
	//myfile1 << "Writing this to a file.\n";
	//myfile1.close();
	/*
	if (myfile.good()) {
	LOG_MSG("DICE QUE ESTA BUENA");
	}
	//myfile.clear();
		LOG_MSG("paso");
	...
	std::string gegevens;
	//gegevens.insert(0, line);
	//LOG_MSG("LINEA%s",getline(myfile,gegevens));
	
	Section* currentsection = NULL;
	Section* testsec = NULL;
	//LOG_MSG("Haber mierda");
	//while (std::getline(myfile,gegevens)) {
	while(fgets(line,sizeof(line), rgb))	{
		gegevens.clear();
		gegevens.insert(0,line);
		//LOG_MSG("test4%s",line);
