...
check_gotbpp:
#endif
		if (sdl.desktop.fullscreen) gotbpp=SDL_VideoModeOK(320,240,32,SDL_FULLSCREEN|SDL_HWSURFACE|SDL_HWPALETTE);
...
static SDL_Surface * GFX_SetupSurfaceScaled(Bit32u sdl_flags, Bit32u bpp) {
	...
	if (fixedWidth && fixedHeight) {
		...
		if (sdl.desktop.fullscreen)
			sdl.surface = SDL_SetVideoMode(320,240,32,sdl_flags);
	...
	case SCREEN_SURFACE:
		...
		if (sdl.desktop.fullscreen) {
			if (sdl.desktop.full.fixed) {
				...
				sdl.surface=SDL_SetVideoMode(320 ,240, 32,
			...
			sdl.clip.x=0;sdl.clip.y=0;
			sdl.surface=SDL_SetVideoMode(320, 240, 32,(flags & GFX_CAN_RANDOM) ? SDL_SWSURFACE : SDL_HWSURFACE);
			//LOG_MSG("PASO1");
#ifdef WIN32
			if (sdl.surface == NULL) {
				...
				sdl.surface = SDL_SetVideoMode(320, 240, 32,SDL_HWSURFACE);
			}
#endif
			if (sdl.surface == NULL)
...
static void GUI_StartUp(Section * sec) {
   ...
   if(sdl.desktop.want_type==SCREEN_OPENGL){ /* OPENGL is requested */
	sdl.surface=SDL_SetVideoMode(320,240,32,SDL_OPENGL);
		if (sdl.surface == NULL) {
	...
	sdl.surface=SDL_SetVideoMode(320,240,32,0);
...
/* Please leave the Splash screen stuff in working order in DOSBox. We spend a lot of time making DOSBox. */
	SDL_Surface* splash_surf = SDL_CreateRGBSurface(SDL_SWSURFACE, 320, 240, 32, rmask, gmask, bmask, 0);
	if (splash_surf) {
		SDL_FillRect(splash_surf, NULL, SDL_MapRGB(splash_surf->format, 0, 0, 0));

		Bit8u* tmpbufp = new Bit8u[320*240*3];
		GIMP_IMAGE_RUN_LENGTH_DECODE(tmpbufp,gimp_image.rle_pixel_data,320*240,3);
...
static void show_warning(char const * const message) {
	...
	printf(message);
	if(textonly) return;
	if(!sdl.surface) sdl.surface = SDL_SetVideoMode(320,240,32,0);
...
int main(int argc, char* argv[]) {
	try {
		...
		std::string editor;
		if(control->cmdline->FindString("-editconf",editor,false)) launcheditor();		if(control->cmdline->FindString("-opencaptures",editor,true)) launchcaptures(editor);
