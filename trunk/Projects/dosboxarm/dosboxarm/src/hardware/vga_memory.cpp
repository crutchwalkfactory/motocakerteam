...
template <class Size>
static void hostWrite(HostPt off, Bitu val) {
	if ( sizeof( Size ) == 1)
...
template <class Size>
static Bitu  hostRead(HostPt off ) {
	if ( sizeof( Size ) == 1)
...
public:
	...
	template <class Size>
	static Bitu readHandler(PhysPt addr ) {
		return hostRead<Size>( &vga.mem.linear[((addr&~3)<<2)+(addr&3)] );
	}
	template <class Size>
	static void writeCache(PhysPt addr, Bitu val) {
	...
	template <class Size>
	static void writeHandler(PhysPt addr, Bitu val) {
