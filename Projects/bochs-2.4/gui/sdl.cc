...
SDL_Surface *sdl_screen, *sdl_fullscreen, *sdl_realscreen;
SDL_Event sdl_event;
...
void switch_to_windowed(void)
{
  ...
  sdl_screen = SDL_SetVideoMode(320,240,32, SDL_SWSURFACE);
...
void switch_to_fullscreen(void)
{
  ...
  dst.x = 0; dst.y = 0;
    res_x=640;
    res_y=480;
  sdl_fullscreen = SDL_CreateRGBSurface(
      SDL_SWSURFACE,
      640,
      480,
      32,
...
/*  SDL_BlitSurface(sdl_screen,&src,tmp,&dst);
  SDL_UpdateRect(tmp,res_x,res_y,0,0);
  SDL_FreeSurface(sdl_screen);
  sdl_screen = NULL;

  sdl_fullscreen = SDL_SetVideoMode(res_x,res_y,32, SDL_SWSURFACE);
  src.y = 0;
    ...
    unsigned header_bar_y)
{
  ...
  //sdl_realscreen=SDL_SetVideoMode(320,240,32,SDL_HWSURFACE|SDL_ASYNCBLIT|SDL_FULLSCREEN|SDL_RESIZABLE);
  switch_to_fullscreen();
  sdl_realscreen=SDL_SetVideoMode(320,240,32,SDL_HWSURFACE|SDL_ASYNCBLIT|SDL_FULLSCREEN|SDL_RESIZABLE);
  //dimension_update(320,240);
...
void sdl_set_status_text(int element, const char *text, bx_bool active, bx_bool w)
{
 ...
 // SDL_UpdateRect(sdl_screen, 320,240,0,0);
}
...
void bx_sdl_gui_c::handle_events(void)
{
  ...
  while(SDL_PollEvent(&sdl_event))
  {
	...
	/*if(sdl_fullscreen_toggle == 0)
	  ...
	  SDL_UpdateRect(sdl_fullscreen, 320, 240, 0,0);
...
void bx_sdl_gui_c::flush(void)
{
  ...
  if(sdl_screen)
	...
	{
	zoomm = sdl_zoom_blit(sdl_fullscreen,sdl_realscreen,SMOOTHING_ON,&rec1);
	SDL_UpdateRect(sdl_realscreen,0,0,320,240);
	}
}
...
/*
  ...
  if(sdl_fullscreen_toggle == 0)
  {
    sdl_screen = SDL_SetVideoMode(320, 240, 18, SDL_SWSURFACE);
    if(!sdl_screen)
    {
      LOG_THIS setonoff(LOGLEV_PANIC, ACT_FATAL);
  ...
  {
    sdl_fullscreen = SDL_SetVideoMode(320, 240, 32, SDL_HWSURFACE|SDL_FULLSCREEN);
    if(!sdl_fullscreen)
    {
      LOG_THIS setonoff(LOGLEV_PANIC, ACT_FATAL);
      BX_PANIC(("Unable to set requested videomode: %ix%i: %s",x,y,SDL_GetError()));
