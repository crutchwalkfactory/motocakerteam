...
void SlaveConfig::deleteEntry( const QString & )
{
    assert( false );
}
...
SlaveBase::SlaveBase( const QCString &protocol, const QCString &, const QCString & )
    : mProtocol( protocol ), m_dcc( new DCOPClient ), m_connection( 0 ),
      m_kconfig( new KConfig( new SlaveConfig( this ), true ) ), 
      m_multipleAuthCaching( false ), m_timeout( 0 )
{
}
...
void SlaveBase::dispatchLoop()
{
    ...
    while ( 42 )
    {
	if (m_timeout && (m_timeout < time(0))) { 
	    QByteArray data = m_timeoutData;
	    m_timeout = 0;
	    m_timeoutData = QByteArray();
	    special(data);
	}
        FD_ZERO( &fds );
...
void SlaveBase::totalSize( KIO::filesize_t s )
{
    KIO_ARGS << KIO_FILESIZE_T(s);
    m_connection->send( INF_TOTAL_SIZE, data );
}

void SlaveBase::processedSize( KIO::filesize_t s )
{
    KIO_ARGS << KIO_FILESIZE_T(s);
...
{
    m_connection->send( INF_ERROR_PAGE );
}
