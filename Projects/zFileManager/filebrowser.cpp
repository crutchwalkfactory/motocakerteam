...
void FileBrowser::setDir(const QString &path)
{
  ...
  while (it != entries.end()) {
    if (*it != ".") {
	  ...
	  QString icono="fm_folder_small";
      QPixmap pixmap=QPixmap(iconReader.getIcon(icono, false));
	  if ( pixmap.height() > 16 ) { QImage image; image = pixmap.convertToImage(); 
	  image = image.smoothScale(16, 16); pixmap.convertFromImage(image); }     
	  pixmap.load(icono);
  ...
  while (it != entries.end()) {
    if (*it != ".") {
      ZListBoxItem* Item = new ZListBoxItem ( this, QString ( "%I%M" ) );
      Item->setPixmap ( 0, QPixmap ( le->ProgramDir + "img/file.png") );
      Item->appendSubItem ( 1, *it, false, NULL);
      this->insertItem ( Item,-1,true );
    }
    ++it;
