...
int CheckCdrom() {
	...
	SysPrintf("*PCSX*: CdromLabel: %s", CdromLabel);
	SysPrintf("*PCSX*: CdromId: %s", CdromId);
...
int Load(const char *ExePath) {
    ...
    switch (type) {
    	case PSX_EXE:
			...
			psxRegs.GPR.n.sp = tmpHead.s_addr; 
			if (psxRegs.GPR.n.sp == 0) psxRegs.GPR.n.sp = 0x801fff00;
			SysMessage("Pcsx found that you wanna use a PSX file.");
	        break;
    	case CPE_EXE:
    		SysMessage("Pcsx found that you wanna use a CPE file. CPE files not supported");
			break;
    	case COFF_EXE:
    		SysMessage("Pcsx found that you wanna use a COFF file. COFF files not supported");
			break;
    	case INVALID_EXE:
...
const char PcsxHeader[32] = "STv3 PCSX v1.5";
